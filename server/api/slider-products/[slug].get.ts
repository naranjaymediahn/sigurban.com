  ...
  try {
    ...
    const rows = await query<any>(
      `SELECT id, slug, name_es, name_en, image, sort_order, is_active,
              ...
              format_es, shelf_life_es, store_temp, units_per_case, logistics_es, gallery_json
    ...
    return { ok: true, data: { ...rows[0], gallery } }
  } catch (err: any) {
    const modelos = await listFallbackSliderProducts(false)
    const found = modelos.find((m: any) => m.slug === slug)
    if (found) return { ok: true, fallback: true, data: { ...found, gallery: found.gallery || [] } }
